...
- **Local MCP Server (`server.py`)**: Communicates with the orchestrator over a secure stdio transport. Exposes **58 OS-control tools** grouped across files, shell command exec, mouse/keyboard inputs, window focal states, screen vision, browser interactions, search/grep, and git tools.
