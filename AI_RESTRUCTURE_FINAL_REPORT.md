...
- 0 errors
- 1 warning — **pre-existing baseline** (unused `_e` at lib.rs:409, present on master before this branch)
...
```
pytest tests/ --ignore=tests/test_browser_protocol.py \
              --ignore=tests/test_gemini_user_project_mode.py \
              --ignore=tests/test_github_create_repo.py \
              --ignore=tests/test_web_resolver.py \
              --ignore=tests/test_web_wait_for_url.py
```

**Result:** `88 passed, 1 skipped in 0.42s`

**5 collection errors (baseline — same on master):**
```
ERROR tests/test_browser_protocol.py
ERROR tests/test_gemini_user_project_mode.py
ERROR tests/test_github_create_repo.py
ERROR tests/test_web_resolver.py
ERROR tests/test_web_wait_for_url.py
```

Root cause: `ModuleNotFoundError: No module named 'yaml'` — `pyyaml` not installed in the test environment. These errors exist on master before this branch. Not introduced by this restructure.

### 4. TypeScript Errors

```
cd desktop && npx tsc --noEmit 2>&1 | grep -v TS2307 | grep -v TS7026 | grep -v TS2875 | wc -l
```

**Baseline (master):** 228 non-TS2307/TS7026/TS2875 errors
**Post-restructure:** 228 errors
**Delta: 0** — no regression introduced.

Note: TS2307 / TS7026 / TS2875 are pre-existing errors from missing type declarations (`qrcode.react`, etc.) that predated this branch and are excluded from comparison.
...
| `tests/test_browser_protocol.py` + 4 others | Broken due to missing `pyyaml` in test env — pre-existing, not this branch's fault, but should be fixed |
