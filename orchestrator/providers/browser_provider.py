...
class BrowserProvider(BaseProvider):
    ...
    async def _verify_injection(self, page: Page, selector: str, expected: str) -> bool:
        ...
        if len(actual_norm) < max(_VERIFY_MIN_CHARS, int(len(expected_norm) * 0.98)):
            logger.warning(f"Injection failed: actual length {len(actual_norm)} "
                           f"is < 98% of expected {len(expected_norm)}")
            return False
        ...
        if not fuzzy_match(actual_norm[:200], expected_norm[:200]):
            logger.warning(f"Injection failed: Prefix mismatch. "
                           f"Actual: {actual_norm[:50]}... Expected: {expected_norm[:50]}...")
            return False

        if not fuzzy_match(actual_norm[-200:], expected_norm[-200:]):
            logger.warning(f"Injection failed: Suffix mismatch. "
                           f"Actual: ...{actual_norm[-50:]} Expected: ...{expected_norm[-50:]}")
            return False
    ...
    async def _wait_for_generation_complete(
        ...
        completion_hash: str | None, site: str = "AI", min_index: int = 0
    ) -> None:
        """
        ...
        while loop.time() < deadline:
            ...
            try:
                ...
                logger.debug(f"[DEBUG] _wait_for_generation_complete text "
                             f"(len={len(current_text)}): {current_text[-100:]!r}")
