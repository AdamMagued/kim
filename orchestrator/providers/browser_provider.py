...
class BrowserProvider(BaseProvider):
        ...
        for attempt in range(1, _INJECT_MAX_RETRIES + 1):
            ...
            await page.evaluate(
                """([selector, text]) => {
                    ...
                    el.focus();
                    if ('value' in el) {
                        const win = window.HTMLTextAreaElement;
                        const setter = Object.getOwnPropertyDescriptor(win.prototype, "value")?.set;
                        if (setter) {
                            setter.call(el, text);
                        } else {
                            el.value = text;
    ...
    async def _verify_injection(self, page: Page, selector: str, expected: str) -> bool:
        ...
        if len(actual_norm) < max(_VERIFY_MIN_CHARS, int(len(expected_norm) * 0.98)):
            logger.warning(
                f"Injection failed: actual {len(actual_norm)} < 98% expected {len(expected_norm)}"
            )
            return False
