...
class BrowserProvider(BaseProvider):
    ...
    async def _complete_via_webview_bridge(
        ...
        try:
            ...
            return {
                "type": "text",
                "content": (
                    "NEED_HELP: Bridge /v1/send timed out. The prompt may have been partially "
                    "sent. Please check the browser window and retry if needed."
                ),
            }
        except Exception as e:
    ...
    async def _verify_injection(self, page: Page, selector: str, expected: str) -> bool:
        ...
        if len(actual_norm) < max(_VERIFY_MIN_CHARS, int(len(expected_norm) * 0.98)):
            logger.warning(
                f"Injection failed: actual length {len(actual_norm)} is < 98% of "
                f"expected {len(expected_norm)}"
            )
            return False
