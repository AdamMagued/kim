...
class BrowserProvider(BaseProvider):
    ...
    async def _complete_via_webview_bridge(
        ...
        try:
            ...
            return {
                "type": "text",
                "content": ("NEED_HELP: Bridge /v1/send timed out. The prompt may have "
    ...
    async def _verify_injection(self, page: Page, selector: str, expected: str) -> bool:
        ...
        if len(actual_norm) < max(_VERIFY_MIN_CHARS, int(len(expected_norm) * 0.98)):
            logger.warning(
                f"Injection failed: actual length {len(actual_norm)} is < 98% "
                f"of expected {len(expected_norm)}"
            )
            return False
        ...
        if not fuzzy_match(actual_norm[:200], expected_norm[:200]):
            logger.warning(
                f"Injection failed: Prefix mismatch. Actual: {actual_norm[:50]}... "
        ...
        if not fuzzy_match(actual_norm[-200:], expected_norm[-200:]):
            logger.warning(
                f"Injection failed: Suffix mismatch. Actual: ...{actual_norm[-50:]} "
                f"Expected: ...{expected_norm[-50:]}"
            )
            return False
    ...
    async def _wait_for_generation_complete(
        self, page: Page, stop_selectors: list[str], response_selectors: list[str], completion_hash: str | None,
        site: str = "AI", min_index: int = 0
    ) -> None:
        """
        ...
        while loop.time() < deadline:
            ...
            try:
                ...
                logger.debug(
                    f"[DEBUG] _wait_for_generation_complete text (len={len(current_text)}): "
                    f"{current_text[-100:]!r}"
                )
