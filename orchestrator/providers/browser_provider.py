...
class BrowserProvider(BaseProvider):
            ...
            await page.evaluate(
                """([selector, text]) => {
                    ...
                    if ('value' in el) {
                        ...
                        const nativeInputValueSetter = pd?.set;
    ...
    async def _verify_injection(self, page: Page, selector: str, expected: str) -> bool:
        ...
        if len(actual_norm) < max(_VERIFY_MIN_CHARS, int(len(expected_norm) * 0.98)):
            logger.warning(f"Injection failed: actual length {len(actual_norm)} is < 98% "
    ...
    async def _wait_for_generation_complete(
        ...
        while loop.time() < deadline:
            ...
            try:
                ...
                logger.debug(f"[DEBUG] _wait_for_generation_complete text (len={len(current_text)}): "
