...
|------|-------|--------|----------|-------|
| `tests/test_browser_protocol.py` | 8 | Codex bridge response mapping + BrowserProvider transport marker behavior | PASS | BrowserProvider tests skip if Playwright is missing |
| `tests/test_browser_provider_parse.py` | 1 | Browser provider response parsing | SKIP | Requires playwright |
| `tests/test_browser_split.py` | 35 | Extracted browser provider modules | PASS | No Playwright/network required |
| `tests/test_contracts.py` | 28 | Stdout/provider/message contracts | PASS | Allows missing provider credentials |
| `tests/test_context_meter.py` | 5 | Context window token counting | PASS | |
| `tests/test_interaction_policy.py` | 7 | Tool execution safety policy | PASS | |
| `tests/test_ollama_provider.py` | 8 | Ollama provider response normalization | PASS | |
| `tests/test_web_resolver.py` | 10 | Web tool URL resolution | PASS | |
| `tests/test_web_wait_for_url.py` | 3 | Web tool URL wait condition | PASS | |
| `tests/test_gemini_oauth_provider.py` | 3 | Gemini OAuth token handling | PASS | |
| `tests/test_gemini_user_project_mode.py` | 14 | Gemini user/project config | PASS | |
| `tests/test_github_create_repo.py` | 3 | GitHub repo creation | PASS | |
