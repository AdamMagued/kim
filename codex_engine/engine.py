...
def _chatgpt_terminal_system_prompt() -> str:
    ...
    return (
        ...
        "- When starting background dev servers or launcher scripts (e.g., dev:up, http.server), "
        "always run them in the background (e.g. `nohup npm run dev:up >/dev/null 2>&1 &`) "
        "so the process stays alive, and do NOT re-run `dev:down` or search files unless a real error is output.\n"
